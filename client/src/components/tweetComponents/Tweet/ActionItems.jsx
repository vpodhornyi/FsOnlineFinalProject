...
const ActionItems = ({ actions, replyCounter, tweetId, user }) => {
  ...
  const actionsObj = {
    LIKE: {
      counter: 0,
      color: "#f91880",
      handler: () => isAuth ? changeAction("LIKE") : navigate(`${PATH.AUTH.ROOT}/${PATH.AUTH.SING_IN.LOGIN}`, {state: {background: location}}),
      userBol: false,
    },
    RETWEET: {
      counter: 0,
      color: "#00ba7c",
      handler: () => isAuth ? changeAction("RETWEET") : navigate(`${PATH.AUTH.ROOT}/${PATH.AUTH.SING_IN.LOGIN}`, {state: {background: location}}),
      userBol: false,
    },
    BOOKMARK: {
      counter: 0,
